...
def validate_story(story: dict[str, Any]) -> list[str]:
    ...
    for field in ("characters", "supporting_assets"):
        ...
        for i, asset in enumerate(value):
            ...
            if field == "supporting_assets" and asset.get("category") not in ASSET_CATEGORIES - {"characters"}:
                errors.append(f"{p}.category must be one of people, animals, locations, props")
